...
function unFinishedAjaxQuery() {
    ...
    $.ajax({
        type: "GET",
        url: rootPath+'/scrmapp/qyhuser/orders/finished/detail',
        data: ajaxData,
        success: function(data) {
            ...
            if (data.returnCode === 1) {
                ...
                if (data.data.faultImage) {
                    ...
                    // $('.imgBox .img img').eq(0).attr('src', faultImage[0]);
                    // $('.imgBox .img img').eq(1).attr('src', faultImage[1]);
                }
                 previewImage()
...
function unFinishedRenderFunc(data) {
    var htmlStr = template("orderDetailTemplate", data);
