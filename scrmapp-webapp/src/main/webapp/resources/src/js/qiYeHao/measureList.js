...
var partData = getUrlParam("partData");
var pdtData = sessionStorage.pdtData,
	pdtData = JSON.parse(pdtData);
function measureList(){
	$.showLoading()
    $.ajax({
        url: rootPath + '/scrmapp/qyhuser/orders/repairItem/get',
        type: 'GET',
        dataType:'JSON',
        data:pdtData,
        success:function(res){
            ...
            if (!data.data) {
            	...
            	$("body").html('<div class="errorIcon"></div><p class="errorTxt">抱歉，没有查到该产品对应的维修措施信息</p>')
            	
            } else {
            	measureListRender(data)
            }
        },
        error:function(){
            //console.log(9999)
...
//选择维修项目
function chooseMeasureItem(){
	$('.measureListItem').click(function(event) {
		var measureData = {
			id:$(this).attr('id'),
			name:$(this).children('.measureTxt').html(),
			itemCode:$(this).attr('itemcode')
		}
		sessionStorage.measureData=JSON.stringify(measureData)
		console.log(sessionStorage.measureData)
		window.location.href=rootPath + '/mobile/qiYeHao/submitOrder/jsp/submit_repair.jsp?ordersCode='+ordersCode+'&orderType='+orderType+'&userId='+userId+'&pageFlag=1' ;
		
	});
}

//渲染维修措施列表
function measureListRender(data) {
    var htmlStr = template("measureListTemplate", data);
