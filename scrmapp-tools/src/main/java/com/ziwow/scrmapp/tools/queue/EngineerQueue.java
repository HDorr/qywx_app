...
public class EngineerQueue<T> implements InitializingBean, DisposableBean {
	private static final Logger LOGGER = LoggerFactory.getLogger(EngineerQueue.class);
	...
	class ListenerThread extends Thread {
		...
		public void run() {
			while (true) {
				...
				try {
					Thread.sleep(1);
					obj = EngineerQueue.queue.poll();
					// 逐个执行
					if (obj != null) {
						listener.onMessage((T) obj);
					}
				} catch (Exception e) {
					LOGGER.error("poll from engineer qunue exception", e);
					//EngineerQueue.queue.add(obj);
				}
			}
		}
	}
