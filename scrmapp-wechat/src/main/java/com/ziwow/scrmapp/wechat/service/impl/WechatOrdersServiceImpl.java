...
public class WechatOrdersServiceImpl implements WechatOrdersService {
  ...
  public Result geneatorCode(WechatOrdersParam wechatOrdersParam) {
    ...
    if (StringUtils.isBlank(orderTime)) {
      ...
      acceptanceFormParam
          .setAppeal_content(wechatOrdersParam.getDescription() + " 预约时间：" + orderTime);
    ...
    acceptanceFormParam.setFix_org_name(
        wechatOrdersParam.getDepartmentName() == null ? "" : wechatOrdersParam.getDepartmentName());
  ...
  public WechatOrdersVo getVoByOrdersCode(String ordersCode) {
    ...
    //查询受理记录
    List<WechatOrdersRecordVo> wechatOrdersRecordList = wechatOrdersRecordMapper
        .findByOrdersId(wechatOrdersVo.getId());
    wechatOrdersVo.setWechatOrdersRecordList(wechatOrdersRecordList);

    //产品列表
    List<ProductVo> productVos = productMapper
        .selectByOrdersIds(new Long[]{wechatOrdersVo.getId()});
    for (int i = 0; i < productVos.size(); i++) {
      ...
      if (StringUtil.isNotBlank(productBarCode) && StringUtil.isNotBlank(productBarCodeTwenty)
          && productBarCode.length() == 20
          && productBarCodeTwenty.length() == 20) {
        productVos.get(i).setProductBarCodeTwenty(
            productBarCode.substring(0, 1) + productBarCodeTwenty.substring(1));
      }
    }
  ...
  public void syncHistoryAppInfo(String mobilePhone, String userId) {
    try {
      ...
      for (int type : list) {
        List<ProductAppealVo> appealList = thirdPartyService
            .getCssHistoryAppInfo(mobilePhone, type);
        ...
        if (null != appealList && !appealList.isEmpty()) {
          for (ProductAppealVo appealVo : appealList) {
            ...
            if (null == wechatOrders) {
              ...
              wechatOrders.setCreateTime(
                  DateUtil.StringToDate(appealVo.getCreateTime(), DateUtil.YYYY_MM_DD_HH_MM_SS));
              wechatOrders.setUpdateTime(
                  DateUtil.StringToDate(appealVo.getCreateTime(), DateUtil.YYYY_MM_DD_HH_MM_SS));
              wechatOrders.setOrderTime(
                  DateUtil.StringToDate(appealVo.getServiceTime(), DateUtil.YYYY_MM_DD_HH));
              ...
              wechatOrdersRecord.setRecordTime(
                  DateUtil.StringToDate(appealVo.getCreateTime(), DateUtil.YYYY_MM_DD_HH_MM_SS));
  ...
  public Result updateOrdersTime(String ordersCode, Date date, String ordersTime) {
    Result updateResult = thirdPartyService
        .updateCssAppeal(ordersCode, ordersTime.substring(0, 10), 1);
  ...
  public void sendWechatOrderTemplateMsg(WechatOrderMsgVo wechatOrderMsgVo) {
    ...
    wechatTemplateService
        .reservationServiceRemind(openId, msgUrl, title, name, serverType, serverTime, remark);
  }
  ...
  public void syncAddAppraise(AppraiseParam appraiseParam) throws ParamException {
    boolean isLegal = SignUtil
        .checkSignature(appraiseParam.getSignature(), appraiseParam.getTimeStamp(),
            Constant.AUTH_KEY);
    if (!(isLegal)) {
...
//        String is_order = checkNull(appraiseParam.getIs_order());
    ...
    qyhUserAppraisalVo.setIs_order(
        StringUtils.isEmpty(is_order) ? null : convertBoolean(is_order));//400回访is_order为非必填字段
    ...
    String engineerMsgContent =
        "您服务的工单" + ordersCode + "，用户已经评价啦，谢谢提供服务！请登录“沁园服务之家”的售后服务个人中心，可以查看评分。";
  ...
  @Override
  public void sendEngineerMsgText(String engineerId, String engineerPhone,
    ...
    String msgContent =
        "您收到一条新工单！类型：" + serviceType + "；服务时间：" + orderTime + "。您可进入“沁园”WX企业号查看该工单详情！";
    qyhUserMsgVo.setMsgContent(msgContent);
    EngineerQueue.getQueueInstance().add(qyhUserMsgVo);
  }

  @Override
  public void changeAppointmentTemplate(String userId, String orderType, String ordersCode,
      String orderTime, String qyhUserName, String qyhUserPhone) {
    WechatFans wechatFans = wechatFansService.getWechatFansByUserId(userId);
    if (null != wechatFans) {
      String openId = wechatFans.getOpenId();
      String nickName = wechatFans.getWfNickName();
      ...
      wechatTemplateService
          .changeAppointmentTemplate(openId, null, title, orderType, ordersCode, orderTime,
              qyhUserName, qyhUserPhone, remark);
    }
  }
  ...
  public void dispatchDot(DispatchDotParam dispatchDotParam) throws SQLDataException {
    ...
    if (wechatOrders == null) {
      ...
      date.setMinutes(0);
      date.setSeconds(0);
      wechatOrders.setOrderTime((StringUtils.isEmpty(ordersTime)) ? date
          : DateUtil.StringToDate(ordersTime, "yyyy-MM-dd"));
  ...
  public void dispatchMaster(DispatchMasterParam dispatchMasterParam) throws Exception {
    ...
    if (StringUtils.isEmpty(acceptNumber) || StringUtils.isEmpty(engineerId) || StringUtils
        .isEmpty(engineerName)) {
  ...
  public void dispatchCompleteOrder(DispatchOrderParam dispatchOrderParam) throws Exception {
    ...
    int count = wechatOrdersMapper
        .updateOrdersNoAndStatus(acceptNumber, finishNumber, date, SystemConstants.COMPLETE);
    if (count > 0) {
      // 录入受理记录表
      WechatOrdersRecord wechatOrdersRecord = new WechatOrdersRecord();
      wechatOrdersRecord.setOrderId(wechatOrders.getId());
  ...
  public void syncMakeAppointment(String scOrderItemId, String ordersCode,
    ...
    LOG.info("预约信息同步到小程序,scOrderItemId:" + scOrderItemId + "   ordersCode:" + ordersCode
    ...
    String result = HttpClientUtils
        .postJson(makeAppointmentUrl, JSONObject.fromObject(params).toString());
  ...
  public void updateMakeAppointment(String oldOrdersCode, String newOrdersCode) {
    ...
    params.put("signture", MD5.toMD5(Constant.AUTH_KEY + timestamp));
    String result = HttpClientUtils
        .postJson(updatemakeAppointmentUrl, JSONObject.fromObject(params).toString());
    if (StringUtils.isNotBlank(result)) {
      JSONObject o1 = JSONObject.fromObject(result);
      if (o1.containsKey("errorCode") && o1.getInt("errorCode") == 200) {
        String fAid = o1.getString("data");
  ...
  public void cancelMakeAppointment(String ordersCode) {
    ...
    params.put("signture", MD5.toMD5(Constant.AUTH_KEY + timestamp));
    String result = HttpClientUtils
        .postJson(cancelmakeAppointmentUrl, JSONObject.fromObject(params).toString());
    if (StringUtils.isNotBlank(result)) {
      JSONObject o1 = JSONObject.fromObject(result);
      if (o1.containsKey("errorCode") && o1.getInt("errorCode") == 200) {
        String fAid = o1.getString("data");
